...
```{r}
#lmod_ = gls()
#plot_data_gls = data.frame("residuals" = lmod_, "y_hat" = y_hat)
#cor(residuals[-1],residuals[-length(residuals)])
...
#data_i = data_i[-outliers,]
#dim(data_i)
#dim(selected_matrix)
#length(colnames(selected_matrix))
main_dataset = cbind(selected_matrix,data_i)
#length(colnames(main_dataset))

lmod_final = lm(SalePrice ~ . , data = main_dataset)
plot_data_s = data.frame("residuals" = lmod_final$residuals, "y_hat" = lmod_final$fitted.values)
...
```
```{r}
summary(lmod_final)$coefficients
```
